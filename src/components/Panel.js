...
class Panel extends Component {
  ...
  renderToggleIcon(collapsed) {
    if (this.props.toggleable) {
      ...
      return (
        <button className="p-panel-header-icon p-panel-toggler p-link" onClick={this.toggle} id={id} aria-controls={ariaControls} aria-expanded={!collapsed} role="tab">
  ...
  renderHeader(collapsed) {
    ...
    const titleElement = (
      <span className="p-panel-title" id={this.state.id + '_header'}>
        {header}
      </span>
    );
    const iconsElement = (
  ...
  renderContent(collapsed) {
    ...
    return (
      <CSSTransition
        ...
        classNames="p-toggleable-content flex-grow-0 flex-grow-1 flex flex-column"
        timeout={{ enter: 1000, exit: 450 }}
        in={!collapsed}
        unmountOnExit
        options={this.props.transitionOptions}
      >
        <div ref={this.contentRef} className="p-toggleable-content flex-grow-0 flex-grow-1 flex flex-column" aria-hidden={collapsed} role="region" id={id} aria-labelledby={this.state.id + '_header'}>
          <div className="p-panel-content flex-grow-0 flex-grow-1 flex flex-column">{this.props.children}</div>
        </div>
      </CSSTransition>
    );
  }
  ...
  render() {
    const className = classNames('p-panel p-component flex-grow-0 flex-grow-1 flex flex-column', { 'p-panel-toggleable': this.props.toggleable }, this.props.className);
